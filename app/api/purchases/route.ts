...
export async function GET() {
  ...
  const allExtraIds = [...new Set(purchases.flatMap((p: PurchaseRow) => p.additionalGoalIds))];
  const extraGoals = allExtraIds.length
    ...
    : [];
  type ExtraGoal = (typeof extraGoals)[number];
  const extraGoalMap = Object.fromEntries(extraGoals.map((g: ExtraGoal) => [g.id, g.name]));
  const withExtraNames = purchases.map((p: PurchaseRow) => ({
    ...p,
    additionalGoalNames: p.additionalGoalIds.map((id: string) => extraGoalMap[id]).filter(Boolean),
  }));
...
export async function POST(req: Request) {
  ...
  const goalItems =
    allGoalIds.length > 0
      ...
      : [];
  type GoalItem = (typeof goalItems)[number];
  const goalsForAi = goalItems.map((g: GoalItem) => ({ name: g.name, price: g.price, savedAmount: g.savedAmount }));
